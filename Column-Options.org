...
| focusable | true | When set to /false/, clicking on a cell in this column will not select the row for that cell. |
...
| selectable | | If /false/, when a row is selected, the CSS class for selected cells ("selected" by default) is not applied to the cell in this column. |
