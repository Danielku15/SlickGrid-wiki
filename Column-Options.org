...
| focusable || When set to /false/, clicking on a cell in this column will not select the row for that cell. (While /focusable/ itself has no default, the default behavior for columns is equivalent to a /true/ value.) |
